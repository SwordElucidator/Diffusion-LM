...
def create_pre_train_model(data_args):
    config = BertConfig.from_pretrained("bert-base-uncased")
    tokenizer = get_tokenizer(data_args)
    config.vocab_size = len(tokenizer.vocab)
    model = BertNetForPreTraining(config, data_args.input_emb_dim)
...
def pre_train(data_args, data_train):
    model = create_pre_train_model(data_args)
...
if __name__ == '__main__':
    ...
    if args.task == 'train':
        ...
        pre_train(args, create_data(args)[0])
